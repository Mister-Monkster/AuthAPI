...
@celery_app.task()
def send_mail_recovery(token: str, to_email: str, subject: str, text_type='plain'):
    try:
        text = f'Вы запросили сброс пароля вот ссылка для сброса пароля {os.getenv("DOMAIN")}recovery/{token}&token={token}'
        msg = MIMEMultipart()
        msg['Subject'] = subject
        msg['To'] = to_email
        msg['From'] = celery_app.conf.SMTP_USER
        ...
        try:
            smtp_server = smtplib.SMTP_SSL(celery_app.conf.SMTP_HOST, int(celery_app.conf.SMTP_PORT))
            smtp_server.login(celery_app.conf.SMTP_USER, celery_app.conf.SMTP_PASSWORD)
            smtp_server.sendmail(celery_app.conf.SMTP_USER, to_email, msg.as_string())
            smtp_server.quit()
        except Exception as e:
            print(f"Ошибка: {str(e)}")
        redis_client = Redis(host="localhost", port=6379, db=0)
        redis_client.setex(name=token, value=f'{to_email}', time=1800)
        print("Email sent successfully!")
        return f"Email sent to {to_email}"
    except Exception as e:
        print(f"Failed to send email: {str(e)}")
